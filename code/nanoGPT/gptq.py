...
if __name__ == "__main__":
    model = quantizing(output_path=model_name[:-3] + '_q.pt', quantize='gptq.int8')
    model.to(device)
    print(model.state_dict())
    print(model.decoder.layers[0].retention.q_proj.weight)
    print(model.decoder.layers[0].retention.q_proj.weight.shape)
    import os
    import pickle
    from contextlib import nullcontext
    import torch
    ...
    # -----------------------------------------------------------------------------
    start = "\nMy name is" # or "<|endoftext|>" or etc. Can also specify a file, use as: "FILE:prompt.txt"
