...
class ColBlockQuantizedLinear(torch.nn.Module):
    ...
    def pack_weight(self, weight_arr):
        weight_arr = weight_arr.to(device=self.weight.device, copy=True)
        for j in range(self.scales.size(1)):
            weight_arr[:, j * self.tile_cols : (j + 1) * self.tile_cols] /= self.scales[
                :, j : j + 1
            ]
            weight_arr[:, j * self.tile_cols : (j + 1) * self.tile_cols] += self.zeros[
                :, j : j + 1
            ]
        weight_arr = weight_arr.clamp_(min=0, max=2**self.bits - 1).to(dtype=torch.uint8)
        self.weight.zero_()
        for nr in range(self.entries_per_byte):
            self.weight += weight_arr[:, nr :: self.entries_per_byte] << (
                nr * self.bits
            )

    def get_weight(self, dtype=torch.float):
        weight_arr = torch.empty(
        ...
        mask = (1 << self.bits) - 1
        for nr in range(self.entries_per_byte):
            weight_arr[:, nr :: self.entries_per_byte] = (
                (self.weight >> (nr * self.bits)) & mask
            ).float()
    ...
    def forward(self, inp):
        if (
            ...
            return qlinear_4bit_weight(inp, self.weight, self.scales, self.zeros)
        weight_val = self.get_weight(dtype=inp.dtype)
        return torch.nn.functional.linear(inp, weight_val, self.bias)
...
class GPTQQuantizer:
    ...
    def quantize(self):
        ...
        if self.actorder:
            ...
            Q = Q[:, invperm]
        #print('shapes:')
        #print(self.linear_module.weight.shape)
